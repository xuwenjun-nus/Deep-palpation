...
% http://arxiv.org/abs/1509.02971
    ...
    properties
        ...
        maxsteps = 100;     % Max steps per episode
    ...
    methods
        ...
        function obj = DDPG_Solver(nnP, nnQ, optimP, optimQ, dimA, dimO)
            ...
            obj.data.success   = zeros(obj.dsize,1);
            obj.nnP         = nnP;
            obj.nnQ         = nnQ;
            obj.nnPt        = copy(nnP);
            obj.nnQt        = copy(nnQ);
            obj.data.success_rate=zeros(obj.dsize,1);
            obj.data.return=zeros(obj.dsize,1);
            obj.data.collision_count=zeros(obj.dsize,1);
        ...
        function [J, avgL, states] = train(obj)
            ...
            state = obj.mdp.initstate(1,'random',true); % add safety constraints
            ...
            while ~terminal          
                ...
                obs = obj.preprocessS(state); % horizontal 
                action = forward(obj.nnP,obs);   %horizontal vector                      
%                 noise = obj.noise_decay * (noise + mymvnrnd(0, obj.sigma.^2, obj.dimA));
%                  noise = obj.noise_decay * mymvnrnd(0, obj.sigma.^2, obj.dimA);

%  count accumulated collisions 

             for i=1:150  % for safe  exploration  run 50 times  to check
                 noise = obj.noise_decay* obj.sigma.^2.*(2*rand(1,obj.dimA)-1);
%                 if 1 %step<50
%                     noise = 0.01*obj.noise_decay * mymvnrnd(0, obj.sigma.^2, obj.dimA);
%                 else
%                     noise = zeros(1,obj.dimA);
%                 end
                % run a safety check routine 
                
                action = action + noise; % hor
                nextstate = obj.mdp.trans_core(state, action'); % vertical 
                safe=obj.mdp. SafetyCheck(nextstate);
                 if(safe==true)
                     break;
                 end
             end
              safe=obj.mdp. SafetyCheck(nextstate);
                if(safe==false)
                    obj.data.collision_count(obj.t+1)=obj.data.collision_count(obj.t+1)+1;
%                     obj.data.collision_count(obj.t+2)= obj.data.collision_count(obj.t+2)+obj.data.collision_count(obj.t+1);
...
%                 action = obj.mdp.parse(action')';
                ...
                J = J + obj.gamma^(step-1)*reward; % expected return per step
                state = nextstate;
                ...
                if (obj.t > obj.bsize)
                    ...
                    % evaluate policy
                    if(mod(obj.t,10)==0)
                         policy.drawAction = @(s)obj.nnP.forward(obj.preprocessS(s))'; 
                         success_rate=0;J_avr=0;

                        for i=1:10% evaluate 10 times calculate succeess rate
                             [J, success] = evaluate_policies(obj.mdp, 1, 100, policy);
                             if success==true
                                    success_rate=success_rate+1;
                                    obj.data.success(idx,:) = 1;
                             end
                            J_avr=J_avr+J;
                        end
                        obj.data.success_rate(idx)=success_rate/10;
                        obj.data.return(idx)=J_avr/10;
                    end
                end
            end
